class AppTranslations {
  static const Map<String, Map<String, String>> dict = {
    'Hindi': {
      'Your Health Plan': 'आपकी स्वास्थ्य योजना',
      '7-Day Plan': '7-दिन की योजना',
      'Ayurveda': 'आयुर्वेद',
      'Grocery': 'किराना सामान',
      'What Can I Cook?': 'मैं क्या बना सकता हूँ?',
      'Change Language': 'भाषा बदलें',
      'Logout': 'लॉग आउट',
      'Account Profile': 'प्रोफ़ाइल',
      'Reset Profile': 'प्रोफ़ाइल रीसेट करें',
      'Feeling Sick?': 'तबीयत ख़राब है?',
      'Your Health Profile': 'आपकी स्वास्थ्य प्रोफ़ाइल',
      'Basic Information': 'बुनियादी जानकारी',
      'Age:': 'उम्र:',
      'Gender': 'लिंग',
      'Male': 'पुरुष',
      'Female': 'महिला',
      'Other': 'अन्य',
      'Height:': 'ऊँचाई:',
      'Weight:': 'वज़न:',
      'Health Goal': 'स्वास्थ्य लक्ष्य',
      'Weight Loss': 'वज़न कम करना',
      'Weight Gain': 'वज़न बढ़ाना',
      'Maintain Weight': 'वज़न बनाए रखना',
      'Muscle Building': 'मांसपेशियाँ बनाना',
      'Improve Energy': 'ऊर्जा बढ़ाना',
      'Next': 'आगे',
      'Back': 'पीछे',
      'Submit': 'जमा करें',
      'Activity & Diet': 'गतिविधि और आहार',
      'Activity Level': 'गतिविधि स्तर',
      'Diet Type': 'आहार प्रकार',
      'Medical Profile': 'चिकित्सा प्रोफ़ाइल',
      'Any Medical Conditions?': 'कोई चिकित्सा स्थिति?',
      'Any Deficiencies?': 'कोई कमी?',
      'Ayurvedic Profile': 'आयुर्वेदिक प्रोफ़ाइल',
      'Any Ayurvedic Complaints?': 'कोई आयुर्वेदिक शिकायत?',
      'Skip & Gen': 'छोड़ें और बनाएं',
      'Fasting Mode': 'उपवास मोड',
      'Share Diet ID': 'डाइट आईडी साझा करें',
      'Pre-meal Breathing': 'भोजन से पहले श्वास',
      'Cheat Meal Approver': 'चीट मील अप्रूवर',
      'Sweet Craving Logger': 'मीठा खाने की इच्छा ट्रैकर',
      'Sick Mode (Pathya)': 'बीमार मोड (पथ्य)',
      'Sleep Routine Alarm': 'नींद का अलार्म',
      'Dadi Maa Ke Nuskhe': 'दादी मां के नुस्खे',
      'Your Prakriti (Dosha)': 'आपकी प्रकृति (दोष)',
      'Retest': 'फिर से टेस्ट करें',
      'Breathe In': 'सांस अंदर लें',
      'Hold': 'सांस रोकें',
      'Breathe Out': 'सांस बाहर छोड़ें',
      '4-7-8 Breathing': '4-7-8 श्वास अभ्यास',
      'Sleep Routine': 'नींद की दिनचर्या',
      'Mindful Eating': 'माइंडफुल ईटिंग',
      'Take 3 deep breaths before eating to improve digestion.': 'पाचन सुधारने के लिए खाने से पहले 3 गहरी सांसें लें।',
      'Done': 'हो गया',
      
      // Activity Levels
      'Sedentary (Desk Job)': 'गतिहीन (डेस्क जॉब)',
      'Lightly Active': 'हल्का सक्रिय',
      'Moderately Active': 'मध्यम सक्रिय',
      'Very Active': 'बहुत सक्रिय',
      'Extremely Active': 'अत्यधिक सक्रिय',
      
      // Diet Types
      'Vegetarian': 'शाकाहारी',
      'Non-Vegetarian': 'माँसाहारी',
      'Vegan': 'वेगन',
      'Eggetarian': 'अंडाहारी',
      
      // Budget
      'Strict Budget (₹200-400/week)': 'सख्त बजट (₹200-400/सप्ताह)',
      'Moderate (₹400-800/week)': 'मध्यम (₹400-800/सप्ताह)',
      'Flexible (₹800+/week)': 'लचीला (₹800+/सप्ताह)',
      
      // Medical
      'GERD / Acid Reflux': 'जीईआरडी / एसिड रिफ्लक्स',
      'Type 2 Diabetes': 'टाइप 2 मधुमेह',
      'Thyroid (Hypothyroid)': 'थायराइड (हाइपोथायराइड)',
      'Thyroid (Hyperthyroid)': 'थायराइड (हाइपरथायराइड)',
      'High Blood Pressure': 'उच्च रक्तचाप',
      'High Cholesterol': 'उच्च कोलेस्ट्रॉल',
      'Fatty Liver': 'फैटी लीवर',
      'Kidney Stones': 'गुर्दे की पथरी',
      'IBS (Irritable Bowel Syndrome)': 'आईबीएस (इरिटेबल बाउल सिंड्रोम)',
      'None': 'कोई नहीं',
      
      // Ayurvedic Complaints
      'Weak Immunity': 'कमजोर प्रतिरक्षा',
      'Rough / Dry Hair': 'रूखे / सूखे बाल',
      'Hair Fall': 'बालों का झड़ना',
      'Acne / Pimples': 'मुंहासे / फुंसी',
      'Dull Skin': 'बेजान त्वचा',
      'Digestion Issues / Bloating': 'पाचन की समस्या / सूजन',
      'Joint Pain / Stiffness': 'जोड़ों का दर्द / जकड़न',
      'Low Energy / Fatigue': 'कम ऊर्जा / थकान',
      'Poor Sleep / Insomnia': 'खराब नींद / अनिद्रा',
      'Stress / Anxiety': 'तनाव / चिंता',
      'Eye Strain / Weak Eyesight': 'आंखों का तनाव / कमजोर नजर',
      'Frequent Cold & Cough': 'बार-बार सर्दी और खांसी',
      'Water Retention': 'वाटर रिटेंशन',
      'Irregular Periods': 'अनियमित मासिक धर्म',

      // Regions
      'North Indian': 'उत्तर भारतीय',
      'South Indian': 'दक्षिण भारतीय',
      'Gujarati': 'गुजराती',
      'Maharashtrian': 'महाराष्ट्रीयन',
      'Bengali': 'बंगाली',
      'Rajasthani': 'राजस्थानी',
      'North-East': 'उत्तर-पूर्व',
      'Select regions to build your ultimate desi plan': 'अपनी बेहतरीन देसी योजना बनाने के लिए क्षेत्र चुनें',
    },
    'Gujarati': {
      'Start': 'શરૂ કરો',
      'Intermittent Fasting': 'ઇન્ટરમિટન્ટ ફાસ્ટિંગ',
      'Jain Diet': 'જૈન આહાર',
      'Sattvic Fast': 'સાત્વિક ઉપવાસ',
      'Chaturthi Vrat': 'ચતુર્થી વ્રત',
      'Pradosh Vrat': 'પ્રદોષ વ્રત',
      'Navratri Vrat': 'નવરાત્રી વ્રત',
      'Ekadashi Vrat': 'એકાદશી વ્રત',
      'Quick Add Staples': 'ઝડપથી મુખ્ય સામગ્રી ઉમેરો',
      'Enter ingredients you have in your kitchen right now.': 'તમારા રસોડામાં હાલ જે સામગ્રી છે તે લખો.',
      'Tell us what you have': 'તમારી પાસે શું છે જણાવો',
      'Your Health Plan': 'તમારી આરોગ્ય યોજના',
      '7-Day Plan': '7-દિવસની યોજના',
      'Ayurveda': 'આયુર્વેદ',
      'Grocery': 'કરિયાણું',
      'What Can I Cook?': 'હું શું બનાવી શકું?',
      'Change Language': 'ભાષા બદલો',
      'Logout': 'લૉગ આઉટ',
      'Account Profile': 'પ્રોફાઇલ',
      'Reset Profile': 'પ્રોફાઇલ રીસેટ કરો',
      'Feeling Sick?': 'તબિયત ખરાબ છે?',
      'Your Health Profile': 'તમારી આરોગ્ય પ્રોફાઇલ',
      'Basic Information': 'મૂળભૂત માહિતી',
      'Age:': 'ઉંમર:',
      'Gender': 'લિંગ',
      'Male': 'પુરુષ',
      'Female': 'સ્ત્રી',
      'Other': 'અન્ય',
      'Height:': 'ઊંચાઈ:',
      'Weight:': 'વજન:',
      'Health Goal': 'આરોગ્ય લક્ષ્ય',
      'Weight Loss': 'વજન ઘટાડવું',
      'Weight Gain': 'વજન વધારવું',
      'Maintain Weight': 'વજન જાળવવું',
      'Muscle Building': 'સ્નાયુ બનાવવા',
      'Improve Energy': 'ઊર્જા વધારવી',
      'Next': 'આગળ',
      'Back': 'પાછળ',
      'Submit': 'સબમિટ કરો',
      'Activity & Diet': 'પ્રવૃત્તિ અને આહાર',
      'Activity Level': 'પ્રવૃત્તિ સ્તર',
      'Diet Type': 'આહાર પ્રકાર',
      'Medical Profile': 'તબીબી પ્રોફાઇલ',
      'Any Medical Conditions?': 'કોઈ તબીબી સ્થિતિ?',
      'Any Deficiencies?': 'કોઈ ઉણપ?',
      'Ayurvedic Profile': 'આયુર્વેદિક પ્રોફાઇલ',
      'Any Ayurvedic Complaints?': 'કોઈ આયુર્વેદિક ફરિયાદ?',
      'Skip & Gen': 'છોડો અને બનાવો',
      'Fasting Mode': 'ઉપવાસ મોડ',
      'Share Diet ID': 'ડાયટ ID શેર કરો',
      'Pre-meal Breathing': 'ભોજન પહેલાં શ્વાસ',
      'Cheat Meal Approver': 'ચીટ મીલ એપ્રૂવર',
      'Sweet Craving Logger': 'મીઠાની તૃષ્ણા લૉગર',
      'Sick Mode (Pathya)': 'બીમાર મોડ (પથ્ય)',
      'Sleep Routine Alarm': 'ઊંઘ રૂટીન અલાર્મ',
      'Dadi Maa Ke Nuskhe': 'દાદી માં ના નુસ્ખા',
      'Your Prakriti (Dosha)': 'તમારી પ્રકૃતિ (દોષ)',
      'Retest': 'ફરી ટેસ્ટ',
      'Breathe In': 'શ્વાસ લો',
      'Hold': 'રોકો',
      'Breathe Out': 'શ્વાસ છોડો',
      'Close': 'બંધ કરો',
      'Your Personalized Ayurvedic Diet': 'તમારો વ્યક્તિગત આયુર્વેદિક આહાર',
      'Generate My Plan': 'મારી યોજના બનાવો',
      'years': 'વર્ષ',
      'Budget': 'બજેટ',
      'Regional Cuisine': 'પ્રાદેશિક ભોજન',
      'Select your region': 'તમારો વિસ્તાર પસંદ કરો',
      'Mindful Eating': 'માઇન્ડફુલ ઇટિંગ',
      'Take 3 deep breaths before eating to improve digestion.': 'પાચન સુધારવા માટે ખાતા પહેલા 3 ઊંડા શ્વાસ લો.',
      'Done': 'થઈ ગયું',
      'Daily Calorie Target': 'દૈનિક કેલરી લક્ષ્ય',
      'Weekly Budget': 'સાપ્તાહિક બજેટ',
      'Daily Hydration': 'દૈનિક હાઇડ્રેશન',
      'Mood today:': 'આજનો મૂડ:',
      
      // Activity
      'Sedentary (Desk Job)': 'બેઠાડુ (ડેસ્ક જોબ)',
      'Lightly Active': 'થોડું સક્રિય',
      'Moderately Active': 'મધ્યમ સક્રિય',
      'Very Active': 'ખૂબ સક્રિય',
      'Extremely Active': 'અત્યંત સક્રિય',
      
      // Diet
      'Vegetarian': 'શાકાહારી',
      'Non-Vegetarian': 'માંસાહારી',
      'Vegan': 'વેગન',
      'Eggetarian': 'ઈંડાહારી',
      
      // Budget
      'Strict Budget (₹200-400/week)': 'ચુસ્ત બજેટ (₹200-400/અઠવાડિયું)',
      'Moderate (₹400-800/week)': 'મધ્યમ (₹400-800/અઠવાડિયું)',
      'Flexible (₹800+/week)': 'લવચીક (₹800+/અઠવાડિયું)',
      
      // Medical
      'GERD / Acid Reflux': 'GERD / એસિડ રિફ્લક્સ',
      'PCOS / PCOD': 'PCOS / PCOD',
      'Type 2 Diabetes': 'ટાઈપ 2 ડાયાબિટીસ',
      'Thyroid (Hypothyroid)': 'થાઇરોઇડ (હાઇપોથાઇરોઇડ)',
      'Thyroid (Hyperthyroid)': 'થાઇરોઇડ (હાઇપરથાઇરોઇડ)',
      'High Blood Pressure': 'હાઈ બ્લડ પ્રેશર',
      'High Cholesterol': 'હાઈ કોલેસ્ટ્રોલ',
      'Fatty Liver': 'ફેટી લિવર',
      'Kidney Stones': 'કિડનીની પથરી',
      'IBS (Irritable Bowel Syndrome)': 'IBS (ઇરિટેબલ બાઉલ સિન્ડ્રોમ)',
      'None': 'કોઈ નહીં',
      
      // Ayurvedic Complaints
      'Weak Immunity': 'નબળી રોગપ્રતિકારક શક્તિ',
      'Rough / Dry Hair': 'રફ / સૂકા વાળ',
      'Hair Fall': 'વાળ ખરવા',
      'Acne / Pimples': 'ખીલ / ફોડલીઓ',
      'Dull Skin': 'નિસ્તેજ ત્વચા',
      'Digestion Issues / Bloating': 'પાચન સમસ્યાઓ / સોજો',
      'Joint Pain / Stiffness': 'સાંધાનો દુખાવો / જકડાઈ જવું',
      'Low Energy / Fatigue': 'ઓછી ઉર્જા / થાક',
      'Poor Sleep / Insomnia': 'નબળી ઊંઘ / અનિદ્રા',
      'Stress / Anxiety': 'તણાવ / ચિંતા',
      'Eye Strain / Weak Eyesight': 'આંખોમાં તાણ / નબળી દ્રષ્ટિ',
      'Frequent Cold & Cough': 'વારંવાર શરદી અને ઉધરસ',
      'Water Retention': 'વોટર રિટેન્શન',
      'Irregular Periods': 'અનિયમિત માસિક ધર્મ',

      // Regions
      'North Indian': 'ઉત્તર ભારતીય',
      'South Indian': 'દક્ષિણ ભારતીય',
      'Gujarati': 'ગુજરાતી',
      'Maharashtrian': 'મહારાષ્ટ્રીયન',
      'Bengali': 'બંગાળી',
      'Rajasthani': 'રાજસ્થાની',
      'North-East': 'ઉત્તર-પૂર્વ',
      'Select regions to build your ultimate desi plan': 'તમારી શ્રેષ્ઠ દેશી યોજના બનાવવા માટે વિસ્તાર પસંદ કરો',
    }
  };

  static String t(String key, String lang) {
    if (dict.containsKey(lang) && dict[lang]!.containsKey(key)) {
      return dict[lang]![key]!;
    }
    return key;
  }
}